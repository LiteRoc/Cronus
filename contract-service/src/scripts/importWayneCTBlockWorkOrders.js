...
const ids = {
  facility: new Types.ObjectId("69cfa11c41f774bb2a19f6ba"),
  contract: new Types.ObjectId("6a24833acadf08b99044baa0"),
  ct: new Types.ObjectId("6a247c66e56db8fb392144ac"),
};
...
async function run() {
  ...
  for (const item of workOrders) {
    ...
    await connection.collection("assets").updateOne(
      { _id: ids.ct },
      { $addToSet: { workOrders: wo._id } }
    );

    await connection.collection("contracts").updateOne(
      { _id: ids.contract },
