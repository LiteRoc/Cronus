...
test('actual application registration has no admin import/mount or endpoint',async()=>{
 ...
 const fakeRequire=name=>{
  ...
  if(name.includes('/routers/')){ const r=express.Router();r.vendorJsonErrorHandler=r.contactJsonErrorHandler=r.followUpJsonErrorHandler=r.interactionJsonErrorHandler=(_q,_s,next)=>next();return r; }if(name.includes('/middleware/'))return {authenticateToken:(_q,_s,next)=>next()};
